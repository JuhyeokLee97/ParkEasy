...
class ParkingLotRepositoryImpl @Inject constructor() : ParkingLotRepository {

    private val mockParkingLots = listOf(
        ParkingLot(
            id = 1,
            name = "강남역 지하주차장",
            pricePerHour = 2000,
    ...
    override suspend fun getParkingLots(): Result<List<ParkingLot>> {
        return Result.Success(mockParkingLots)
    }

    override suspend fun getParkingLot(id: Int): Result<ParkingLot> {
        val parkingLot = mockParkingLots.find { it.id == id }
        return if (parkingLot != null) {
            Result.Success(parkingLot)
        } else {
            Result.Error(Exception("Parking lot not found"), "Parking lot not found")
        }
    }

    override suspend fun getParkingLotNearby(
        latitude: Double,
        longitude: Double,
        radiusInKm: Double,
    ): Result<List<ParkingLot>> {
        val parkingLot = mockParkingLots
        return if (parkingLot != null) {
            val nearbyParkingLots = modifyParkingLotsPositionToNearby(parkingLot, latitude, longitude, radiusInKm)
            Result.Success(nearbyParkingLots)
        } else {
            Result.Error(Exception("Parking lot not found"), "Parking lot not found")
        }
    }

    private fun modifyParkingLotsPositionToNearby(
        parkingLots: List<ParkingLot>,
        centerLat: Double,
        centerLng: Double,
        radiusInKm: Double,
    ): List<ParkingLot> {
        return parkingLots.mapIndexed { idx, parkingLot ->
